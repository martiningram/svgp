...
def ordered_probit_lik(y, f, cut_points, sd, min_val=-20, fallback=False):
    ...
    def middle_function(f):
        ...
        # But because tensorflow's where evaluates each branch, I hack it here.
        upper_to_select = tf.minimum(ymax - 1, tf.maximum(1, upper_to_select))
        lower_to_select = tf.minimum(ymax - 2, tf.maximum(0, lower_to_select))
